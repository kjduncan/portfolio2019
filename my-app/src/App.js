...
import macroHome from './images/macroHome.png';
import mProject from './images/macroProject.png';
import mHome from './images/LandingF.png';
import mGlobal from './images/globalSearch.png';
import mLabel from './images/Labeling.png';
import mNav from './images/navigation.png';
import mAdd from './images/addFood.png';
import mMagic from './images/macroMagic.png';
import mLanding from './images/landing.png';
import mButtons from './images/buttons.png';
import mProfile from './images/profile.png';
import mRoadmap from './images/roadmap.svg';
import mColorOne from './images/colorSystem1.png';
import mColorTwo from './images/colorSystem2.png';
...
let projects = [
  {
    ...
    attributes: {
        ...
        process: "I started out trying to develop a better understanding of macro counting in general, the current state of the Macrostax application, how users are engaging with it, and <strong>identifying the key pain points that they were experiencing while trying to complete daily tasks.</strong> This took the form of secondary research, interviewing current employees who built the app, interviewing current Macrostax users and performing usability studies, and using the app myself for about 6 weeks.",
        research: 'My research phase began by developing a baseline understanding of the current state of Macrostax and the nutrition and diet tracking industry as a whole.',
        researchS: '<strong>I began with a competitive analysis to identify opportunities and to document heuristics and UI patterns for nutrition tracking. Along with this, I conducted several interviews with those who built Macrostax and those who use Macrostax.</strong> I also interviewed all current employees at Macrostax, asking them what they saw as key features and opportunities. Finally, I spoke with current Macrostax users about their experience and poured over survey data from former customers who had left the app.',
        insightOne: 'Key features are undiscoverable',
        insightOneInfo: 'One of the best features that Macrostax offers its users is tailored macro numbers based on activity levels and the resulting recipe recommendations that will enable users to easily hit their nutrition goals from day to day. <strong>This feature was completely hidden from view and the benefits weren’t clearly explained once a user was able to find it.</strong>',
        insightOneInfoQ: 'Most of the users that I spoke to weren’t even aware that this feature existed. Once I told them about it, they wanted to use it all the time.',
        insightTwo: 'The app is exhausting to interact with and visually inconsistent',
        insightTwoInfo: '<strong>Users struggled to consume key visual components and were left feeling confused and lost most of the time while using the app.</strong> These struggles were commonly associated with small text, lack of color contrast between text color and background color, hidden elements, and unclear iconography.',
        insightTwoInfoQ: 'The app was providing little to no visual affordances for the purpose of key components associated with completing important tasks.',
        insightThree: 'Adding food is unreasonably difficult',
        insightThreeInfo: 'Many expert users had learned pretty extensive work-arounds to make adding food “work” for them. <strong>A task that is key to experiencing success in the app had become so difficult that most users gave up before they had even started.</strong>',
        insightThreeInfoQ: 'This flow lacked clarity, focus, and was generally overwhelming because the app provided little to no guidance or shortcuts.',
        insightFour: 'Lack of feedback left users feeling lost',
        insightFourInfo: 'It was difficult for users to know if the app had received their input. <strong>Once a user completed a task, there were no visual cues that the task had been completed.</strong> This was especially frustrating when a user was adding food and wanted to make sure that the food they were trying to add to their tracker was in fact added to their tracker.',
        insightFive: 'Understanding the Macrostax App',
        insightFiveInfo: 'I spent a lot of time noting features that I thought were the “secret sauce” of macrostax, rating pain points by level of severity and the level of difficulty associated with addressing them, and performing a competitive analysis to uncover potential opportunity to stand out from competitors. A few important things that I wanted to focus on were: improving over all readability and accessibility throughout the app, highlighting the awesome features that most users were overlooking, improving visual consistency, and introducing new features that could enhance the user experience',
        insightFiveP: mLanding,
        insightFivePS: mButtons,
        insightFivePT: mProfile,
        insightSix: 'Prioritizing UX/UI Improvements',
        insightSixInfo: 'With all of this data in hand, <strong>I worked with engineers and company leadership to lay out a roadmap for how we could tackle some key usability concerns while at the same time improving the look and feel of the app and adding new features.</strong> I led the initiative to develop a UI refresh release roadmap along with a more formal product development cycle. We collaborated to determine what workflow would work best for each department and then committed to an iterative product development cycle focused on feature versioning and based on 3 week product development sprints. The idea was to make reasonable timelines and chunk work into 3 week versions with the goal of having a release every 3 weeks.',
        insightSixP: mRoadmap,
        insightSixInfoS:'As we moved forward with the UI refresh project, <strong>we wanted to focus of sustainability in our design and development process.</strong> As part of this, I have begun and continue to work on defining the design language for the product, developing a design system that will be used across the application in order to improve consistency and accessibility, ease the development process, and encourage collaboration between design and engineering. <strong>A design system will help to ensure that the quality of the app is maintained as we scale our product and team in the future as well.</strong>',
        insightSixPS: mColorOne,
        insightSixPT: mColorTwo,
        // insightSeven: 'Development Process',
        // insightSevenInfo: 'Our product development process <strong>begins with defining key functionality and feature requirements.</strong> Based on this, I create low-fidelity mockups and sketches for review. We then quickly iterate until reaching a higher fidelity and then demo to the rest of the company. Barring any huge issues, high fidelity mockups are then handed off to engineering to be built. <strong>Throughout the engineering build, I collaborate closely with the frontend engineer</strong>, making sure that the functionality and visual components meet our predetermined requirements and will be finished by the deadline.',
        // insightSevenInfoS: 'Daily Reflections',
        // insightSevenInfoP: hsDailyProto,
        insightEight: 'Usability Testing',
        insightEightInfo: 'At the end of each development cycle, before we get into QA and the final release, I conduct usability tests with 3-5 users. These usability tests are typically structured as think aloud protocols and <strong>focus on users completing key tasks in the app with the hope of uncovering important issues that impact key task flows.</strong> When issues are discovered, they are rank based on severity, the likelihood that other users will experience the same issue, and the level of difficulty it would be fix them in engineering. I then <strong>offer recommendations to the engineering team as well as company leadership.</strong>',
        insightEightP: mTest,
        result: 'The result is a <strong>refreshed UI that focuses more on accessibility, ease of use, information discovery, and task completion.</strong> There are visual elements that support this result such as typography enhancements, better color contrast, and iconography. There are also <strong>additional user experience features</strong> such as a fixed footer for navigation clarity, a global recipe search engine that highlights Macrostax’s ability to “fill” user’s macro targets as well as a new “add food” flow that provides more clarity and focus around adding food to the tracker.',
        resultOne: mHome,
        resultOneInfo: 'Visual UI components that reduce cognitive load',
        resultOneInfoS: '<strong>Providing more visual clarity has a significant impact on the cognitive load that users may experience while performing a task.</strong> Specifically, I introduced more structure and organization to the food log landing page, <strong>made color changes in order to meet WebAIM accessibility guidelines,</strong> improved readabiilty using font weight, size, and information architecture, and <strong>improved color consistency to indicate meaning for button states and data visualizations.</strong>',
        resultThreeInfo: 'Iconography and labeling to improve scanability ',
        resultThreeInfoS: 'By introducing more help text and labeling on buttons and icons, <strong>the behavior and functionality of interactive elements becomes more clear,</strong> providing orientation and grounding at all stages of interaction.',
        resultTwoInfoSTwo: 'Users are also able to save their favorite recipes to revisit later without having to add them to their food log. By providing these options and surfacing the "fill macros" feature, <strong>users are empowered to tailor their experience and receive support in achieving their health related goals.</strong>',
        resultThree: mLabel,
        resultFourInfo: 'Global and local navigation for focused task completion',
        resultFourInfoS: 'The UI now visually promotes clear separation between global and local navigation. Daily tasks live in a FAB menu, global app navigation lives in the footer component, navigation through the current section of the app happens in the header component. <strong>This promotes tight coupling of navigation elements, focused task completion, and aids in orienting users more effectively while using the app.</strong>',
        resultFour: mNav,
        resultTwoInfo: "Simplified recipe search",
        resultTwoInfoS: "<strong>Macrostax has a robust recipe library</strong> that users can now browse more easily outside of the context of adding food to each individual meal. They are able to <strong>browse all recipes, see a select list of 'featured recipes', or only view recipes that will 'fill' their macro targets for a given day or meal.</strong>",
        resultFive: mAdd,
        resultFiveInfo: 'An improved flow for adding food',
        resultFiveInfoS: 'Users are able to search for and find food items that best meet their needs via a <strong>separate menu and add food flow.</strong>',
        resultFiveInfoSTwo: 'If users are unable to find the foods they are looking for, they can <strong>create and manage custom foods</strong>, specifying ingredients and macronutrients themselves',
        resultSix: mMagic,
        resultSixInfo: 'Macro Magic and Managing Food Log',
        resultSixInfoSTwo: 'Once a food item is added to their food log, <strong>the system offers a notification that something has been added successfully.</strong> Users are then able to see an overview of food items they have added to specific meals, <strong>see how those items impact their macro targets, and make serving size adjustments as needed.</strong>',
        resultSixInfoS: 'When a user is adding food to their food log, they are given the option to <strong>browse a selection of recipes or explore different combinations of individual food items to have their macros "filled" for them.</strong>'
